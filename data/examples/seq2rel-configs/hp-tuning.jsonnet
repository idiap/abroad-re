...
local grad_acc_steps = 10;     // Number of training steps before backpropagating gradients

local decoder_lr = std.parseJson(std.extVar('lr_decoder'));   // Learning rate for decoder params
...
local beam_size = std.parseInt(std.extVar('beam_size'));          // Beam size to use during decoding (test time only)
local length_penalty = std.parseJson(std.extVar('length_penality'));   // >1.0 favours longer decodings and <1.0 shorter (test time only)
...
// These are provided as external variables. YOU NEED TO ADAPT THE PATHS
local train_data_path = "output/examples/generations/seq2rel/out_prompts_example-input.tsv";
local validation_data_path = "data/examples-validation/seq2rel/sub_valid.tsv";
local dataset_size = 33;

// Validation begins at the end of the validation_start epoch...
local validation_start = std.max(std.floor(num_epochs - 1), 0);
