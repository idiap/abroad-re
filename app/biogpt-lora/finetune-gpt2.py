...
parser.add_argument("--epochs-eval-interval", help="Interval between each evaluation on generated outputs on the validation set", type=int, required=False, dest='epochs_interval', default=1) # For unknow reason generatio is much more longer on GPT2-m than on BioGPT
...
LORA_CONFIG = LoraConfig(
    ...
    lora_dropout=0.05,
    fan_in_fan_out=False, # STILL False bc it will be Linear4bit layers after quant.
    bias="none",
    task_type="CAUSAL_LM"
)
...
EVAL_EPOCH_INTERVAL=int(args.epochs_interval)
LIST_EPOCHS_TO_EVAL = np.arange((MIN_EPOCH_BEFORE_EVAL + 1), NUM_EPOCHS , EVAL_EPOCH_INTERVAL).tolist()
...
# Add <BOS> and <PAD> tokens to match the BioGPT settings
tokenizer.add_special_tokens(
    {'additional_special_tokens': ['<|beginoftext|>', '<pad>']}
)
model.resize_token_embeddings(len(tokenizer))
tokenizer.bos_token = '<|beginoftext|>'
tokenizer.bos_token_id = len(tokenizer) - 2 
tokenizer.pad_token = '<pad>'
tokenizer.pad_token_id = len(tokenizer) - 1
EVAL_GENERATION_ARGS["pad_token_id"] = tokenizer.pad_token_id
...
logger = get_logger("qlora-trainer", path=os.path.join(output_dir, "gpt2-medium-training.log"))
...
for epoch in range(NUM_EPOCHS):
    ...
    if ((epoch + 1) > MIN_EPOCH_BEFORE_EVAL) and ((epoch + 1) in LIST_EPOCHS_TO_EVAL) :
